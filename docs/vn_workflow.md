...
`restyle` 只动 `art_direction`：`style` 拿 `house_style` 逐字覆盖，`avoid_style`
拿 `house_avoid` 整段换掉，`avoid` 里只摘掉上一版取向留下的画风词，其余这部作品
自己的忌讳一条不删。它是**点名才跑**的一段，
...
项目默认的取向是**精致国漫融合日系厚涂插画**，写在 `prompts_template/vars.yaml` 的
`house_style`，和它成对的画风负面词写在 `house_avoid`。**画风词只准出现在这一处**。

这条是换过两次取向换出来的。第一次从半写实换到写实，取向词散落在四个地方：
`vars.yaml`、`s3` 的 prompt 骨架（「写实场景」「没有线稿和描边」）、cast 模板
（「不要写线稿厚涂插画风」）、`s2_restyle` 硬编码的反向词。第二次再换回插画，
这四处全都成了**反向指令**——`avoid` 里躺着「厚涂、插画风」，骨架里写着「写实场景」，
和新的 `style` 在同一句 prompt 里互相拆台。所以现在骨架只管画幅、构图、去背，
有一条测试（`test_skeleton_carries_no_style_words`）专门守着它不许再混进画风词。

`style` 里要**把风格化程度钉死**，而不是只写「日系插画」。模型对「眼睛放不放大」
「描边多粗」「头发是分束厚涂还是平涂」这种具体指令有反应，对「精美」「高质量」没有。

负面词在 `art_direction` 里分两段存：`avoid_style` 是画风层的（`house_avoid`，
换取向时整段换掉），`avoid` 是这部作品自己的忌讳（多余人物、畸形手指…），
换画风不该丢。拼 prompt 时两段接起来。

同一个角色的「风格化程度」要**统一写在 `house_style` 里**，别零散写进个别角色的
`desc`：一个角色写了「皮肤有细纹」、别人没写，这个角色就会单独飘成另一种画风。
...
Seedream 不出 alpha，所以背景得自己抠。立绘一律画在**绿幕**（#00B140）前，
按色距抠——绿比红蓝的较大者高出多少，就是这个像素有多「背景」。

一开始用的是白底 + 边缘连通域 flood fill，走不通，原因很本质：**白底下「背景」和
「白衣服」在像素上是同一种东西**。实测踩过的坑，一个比一个隐蔽：

- 白球鞋被从鞋边一路啃进鞋里
- 真丝衬衫的高光（≥242，又被门襟切成小块）被当成发间白洞，一件衬衫打出十几个洞
- 两腿之间那条 31926 像素的白带抠不掉，因为它超出了「小块」的上限
- 米色西装夹住的背景怎么都分不出来

最后一条是死路：想区分「陷落的背景」和「布料高光」，四周亮度（145–198 vs 223–237）、
局部纹理（2.7–3.4 vs 3.8–8.0）、有多白（246 vs 252，还是反的）——三组区间全部重叠。
换成衣服上不可能出现的饱和绿之后，判据退化成一个减法，上面这些全部消失。

绿幕会把绿光反到浅色衣服上，所以抠完要**去溢色**：把绿通道压到红蓝的较大者。
对本来就不发绿的像素这是恒等变换，可以整张无脑做。大面积被染绿救不回来，
所以 prompt 里还得直说「人物身上不带任何绿色调」。

模型原样给的那张（带绿幕的）存在 `vn/stories/<名字>/raw/`。改了去背算法拿它重抠就行，
不用重新花钱生图——绿一旦抠掉就找不回来了，这份留底是唯一的退路。
