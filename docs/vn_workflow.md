...
边在图上连着，玩家却永远看不到。《三杯凉掉的美式》第一版就踩了这个，两个结局是死的。
...
2. **第一个角色的 `normal` 立绘是全局风格锚**：它先画，之后每个角色的 `normal` 都拿它
   当 `reference_image`。不这么做的话三个角色各画各的，很容易一个出成半写实、
   另两个出成卡通，摆进同一张 CG 就穿帮
3. 每个角色的表情图再参考本角色自己的 `normal`

于是依赖链是三层——**风格锚 → 各角色 normal → 各表情**——排程按拓扑一拨一拨推，
拨内并发，不能写死拨数。

`style` 里要**把风格化程度钉死**，而不是只写「日系动画风」。模型对「鼻子画不画鼻孔」
「皮肤画不画毛孔」「睫毛是成组的粗线还是一根根」这种具体指令有反应，对「精美」「高质量」
没有。同理，角色的 `desc` 里不要写「皮肤粗糙」「法令纹很深」这类**把人往写实推**的词——
一个角色写了、别人没写，这个角色就会单独飘出去。
